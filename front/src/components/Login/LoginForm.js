...
function LoginForm() {
    ...
    const validationSchema = Yup.object().shape({
        username: Yup.string().min(3).max(20).required("Bạn hãy nhập tên người dùng"),
        password: Yup.string().min(4).max(20).required("Bạn hãy nhập mật khẩu"),
    });
            ...
            >
                <Box sx={style}>
                    ...
                    <Typography variant="h6" id="modal-modal-description" sx={{ mt: 2 }}>
                        Bạn sẽ có thể đăng nhập và sử dụng các dịch vụ của chúng tôi ngay khi hồ sơ của bạn được quản trị viên xem và phê duyệt!
                    </Typography>
                </Box>
            </Modal>
            ...
            >
                <Box sx={style2}>
                    <div className='container'>
                        ...
                        <button className='buttonito' onClick={handleCloseAlert}>Đóng</button>
                    </div>
                </Box>
            </Modal>
